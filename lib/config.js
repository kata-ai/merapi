...
class Config {
    ...
    constructor(data, {left = "{", right = "}"} = {}, lazy = false) {
    ...
    resolve(path) {
        if (arguments.length) {
            ...
            if (typeof val == "string") {
                ...
                let subset = this.path(path);
                let flat = subset.flatten();
                let ret = {};
                for (let n in flat) {
                    ret[n] = this.set(n, this.resolve(path+"."+n));
                }
                return ret;
            } else {
                return val;
     ...
     *      new config
     */
    static create(data, {left = "{", right = "}"} = {}, lazy = false) {
        ...
        let cfg = new Config(data, {left, right}, lazy);
     ...
     *      new config
     */
    create(data, delimiters, lazy = false) {
        return Config.create(data, delimiters || this.delimiters, lazy);
    }
}
